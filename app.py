...
uploaded_file = st.sidebar.file_uploader("Nahraj obrázek. Neboj, zůstane jen u tebe! \U0000267B", type=["jpg", "jpeg", "png", "JPG", "JPEG", "PNG"])
    ...
    if uploaded_file and st.session_state.show_segment_button:
        if st.button("Segmentovat"):
            with st.spinner("Už asi něco vidím, chvíli strpení... \U0001F441"):
                ...
                segmented_img, labels = segmentation.segment_image(img, api_key_hf)
...
if st.session_state.labels:
    ...
    st.write("Tady je seznam toho, co **Mask2Former** na obrázku rozpoznal. Vyber si jednu z položek v nabídce níže \
             a profesor **Bagrstein** ti o ní poví její evoluční příběh.")
    ...
    st.markdown('<span style="font-size: 15px;"><br>Jen tak mezi námi:<br>**Mask2Former** neumí klasifikovat úplně všechno, co vidí, \
                tak mu to odpusťme. Profesor **Bagrstein** je ovšem odborník, takže zvládne okomentovat i segmenty, \
                které nejsou zrovna středobodem lidské civilizace (třeba obloha, tráva, ...). \
                Nejvíc se ale vyžívá ve výkladu o různých předmětech a artefaktech \
                (jako je vidlička, pohovka, apod.), tak směle do toho!<br><br></span>', unsafe_allow_html=True)

    # Možnost zadat vlastní téma pro generování
    custom_prompt = st.text_area(
        "*Nechceš používat segmentované objekty? Můžeš napsat vlastní téma pro příběh přímo sem ↓*:",
        placeholder="Napiš cokoliv, o čem chceš slyšet evoluční příběh (např. 'kolo', 'mobilní telefon', 'čajová konvice')...",
    ...
    if st.button("Zavolej profesora"):
        ...
        try:
            ...
            if custom_prompt.strip():  # Priorita vlastního promptu
                selected_topic = custom_prompt.strip()
            else:
                selected_topic = selected_class.split("(")[0].strip()  # Ošetření neznámých tříd

            # Vytvoření promptu
            builder = PromptBuilder()
            prompt = builder.build(selected_topic)

            # Volání LLM
            if model_choice == "Perplexity":
                llm_model = llm.PerplexityLLM(api_key=api_key_per)
            else:
                llm_model = llm.OpenAILLM(api_key=api_key_openai)
        ...
        st.markdown('<span style="font-size: 15px;"><br><br>---------------------------------------------------------------\
                    ---------------------------------------------------------------------------------------<br>\
                    *Admin: Jestli máš pocit, že tě profesor moc nepobavil nebo znalostně neobohatil, \
                    zavolej jej znovu. Můžeš si zvolit i jinou položku z nabídky, nebo zadat vlastní téma.*</span>', unsafe_allow_html=True)
        

# Hláška při neúspěšné segmentaci
if st.session_state.segment_attempt:
    st.write("Pokud nic nevidíš, nebo obrázek nemá zakreslené nalezené segmenty, klikni na Reset a pak znova na Segmentovat. \
             Však to znáš z Windows. \U0001F926")
